...
3. Completa las variables marcadas como manuales en el dashboard de cada servicio:
   ...
   - Ambos servicios ya traen `FRONTEND_URL` y `VITE_API_URL` precargadas. **URLs reales de este despliegue:** frontend en `https://devtree-frontend.onrender.com`, backend en `https://devtree-backend-myhe.onrender.com` (Render le agregó el sufijo `-myhe` porque `devtree-backend` ya estaba tomado por otra cuenta). Si vuelves a crear los servicios desde cero y Render asigna nombres distintos, actualiza ambas variables en el dashboard para que coincidan con las URLs reales y vuelve a desplegar — de lo contrario el frontend le pega a una URL que no es tu backend y verás errores de CORS.
